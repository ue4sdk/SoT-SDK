...
{
public:
	ACinematicCameraController*                        CinematicCameraController;                                // 0x0078(0x0008) (ZeroConstructor, IsPlainOldData)
	UClass*                                            CinematicCameraControllerClass;                           // 0x0080(0x0008) (ZeroConstructor, IsPlainOldData)
	unsigned char                                      UnknownData00[0x28];                                      // 0x0088(0x0028) MISSED OFFSET
	TArray<FWorldMarkerDesc>                           CreatedWorldMarkers;                                      // 0x00B0(0x0010) (ZeroConstructor, Transient)
	UGameEventSchedulerSettingsAsset*                  DebugSchedulerSettings;                                   // 0x00C0(0x0008) (ZeroConstructor, Transient, IsPlainOldData)
	AServerPerformanceReplicator*                      ServerPerformanceReplicator;                              // 0x00C8(0x0008) (ZeroConstructor, Transient, IsPlainOldData)
	...
	FShipCustomizationPersistenceModel                 ShipCustomizationModel;                                   // 0x00F8(0x0010) (Transient)
	unsigned char                                      UnknownData02[0x28];                                      // 0x0108(0x0028) MISSED OFFSET
	...
	void Walk();
	void VomitWithType(const FName& VomitType, float Duration);
	void Vomit();
	void UnlockTradeRouteSelectionForMyCrew();
	void UnlockTradeRouteSelectionForCrew(const FGuid& CrewId);
	void UnlockEntitlement(const FString& EntitlementId);
	void UnlockAllEntitlements(int TrueFalse);
	void UnlockAllEmblemsAndAchievements();
	void UnBreakLeg();
	void TutorialAction(const FString& ActionTypeString);
	void TriggerTunnelMigration();
	void TriggerRewardNotification(FName* Identifier);
	void TriggerLandmarkReaction(int ActionType);
	void TriggerFogManagerAtNearestIsland();
	void TriggerEmblemUnlockedMessage(const FString& EmblemFriendlyName);
	...
	void ToggleVideprinter(const FString& Id);
	...
	void TeleportToDebugDestinationLocation(const FString& ActorIdString, const FString& DestinationId);
	...
	void TeleportCrewToShip(const FString& CrewId);
	void TeleportCrewToSafeSpawnLocationFromRemoteActor(const FString& ActorIdString, const FString& CrewId);
	void TeleportCrewToSafeSpawnLocation(const FString& CrewId);
	void TeleportCrewMemberToShip(const FString& ActorIdString);
	...
	void TeleportActorToTrackedActorType(const FString& ActorIdString, int DestinationActorType);
	void TeleportActorToTeleportLocationActor(const FString& ControllerActorIdString, const FString& TeleportLocationActorIdString);
	void TeleportActorToPlayerBuriedDigSite(const FString& ActorIdString);
	void TeleportActorToLocation(const FString& ActorIdString, float LocationX, float LocationY, float LocationZ, float Yaw);
	void TeleportActorToIsland(const FString& ActorIdString, const FString& IslandName);
	void TeleportActorToDigSite(const FString& ActorIdString, bool WasBuriedByPlayer);
	void TeleportActorToActorWithOffset(const FString& ActorIdString, const FString& DestinationActorIdString, float OffsetX, float OffsetY, float OffsetZ);
	void SuperShovel();
	void SuperSailor();
	void StoreWieldedItemInBootyStorageInCurrentIsland();
	void StoreWieldedItemInBootyStorage(const FString& IslandName);
	...
	void StartVoyage(const FString& SourceAssetName, bool Development);
	void StartSicknessOnPlayer();
	void StartSelectedRomeVoyage(const FString& RomeVoyageString);
	...
	void SpawnWatercraft(const FString& WatercraftClassString);
	void SpawnTreasureChestOfType(FString* ChestTypeString);
	void SpawnTreasureArtifact(const FString& TypeString);
	...
	void SpawnStrongholdKeyFromGameEventType(const FString& FortEventName);
	void SpawnStrongholdKey();
	void SpawnSmallShipAtIsland(const FString& IslandName);
	...
	void SpawnShipOfType(float SpawnLocationX, float SpawnLocationY, float SpawnLocationZ, float SpawnYaw, const FString& TypeString);
	void SpawnShipInFrontOfNearestAIInteractable(float XProportion, float YProportion, float Yaw, const FString& TypeString);
	void SpawnShipFromDesc(const FString& InShipDescAssetString, float SpawnLocationX, float SpawnLocationY, float SpawnLocationZ, float SpawnYaw);
	void SpawnShipAtIsland(const FString& IslandName);
	void SpawnShip(float SpawnLocationX, float SpawnLocationY, float SpawnLocationZ, float SpawnYaw);
	void SpawnRomeTreasureChestOfType(const FString& ChestTypeString);
	void SpawnNumberOfAI(const FString& AIDescString, int NumToSpawn);
	void SpawnNightmareAggressiveGhostShipEncountersRandom();
	void SpawnNightmareAggressiveGhostShipEncountersNearToPlayer();
	void SpawnMultipleTreasureChestsOfType(int Num, FString* ChestTypeString);
	void SpawnMessageInABottle(const FString& MessageInABottleTypeString);
	void SpawnMermaid();
	void SpawnMerchantFauna(const FString& FaunaTypeString);
	void SpawnMerchantCrate(const FString& MerchantCrateTypeString);
	void SpawnMerchantCargo(const FString& MerchantCargoTypeString);
	void SpawnKrakenAtCurrentLocationWithNumTentacles(uint32_t NumTentacles);
	void SpawnKrakenAtCurrentLocation();
	void SpawnItemOnFloor(const FString& ItemString);
	void SpawnItemInHand(const FString& ItemString);
	void SpawnGoldMound(const FString& GoldMoundTypeString);
	...
	void SpawnFishAtPlayerLocation(const FString& InBaitType);
	void SpawnFirework(const FString& FireworkItemString);
	void SpawnCursedCannonball(const FString& CannonballTypeString);
	void SpawnContextualPrompt(const FString& PromptAccessKey);
	void SpawnCollectorsChestOfType(FString* ChestTypeString);
	void SpawnCargoRunCrate(const FString& SpawnCargoRunCrateString);
	void SpawnCabinDoorInFrontOfPlayer(float Distance);
	void SpawnBountyReward(const FString& BountyTypeString);
	void SpawnBootyPickupPoint();
	void SpawnBarrelGroup(bool ForcedCloseSpawn);
	void SpawnAThousandTreasureChests();
	void SpawnAndEquipDebugWieldable(const FString& DebugWieldableTypeString);
	void SpawnAINoTrigger(const FString& AIDescString);
	void SpawnAIEncounter(const FString& AIEncounterString);
	void SpawnAIAtNearestAISpawnPoint(const FString& AIDescString);
	void SpawnAIAtLocationDelayed(const FString& AIDescString, float LocationX, float LocationY, float LocationZ, float Yaw, float Delay);
	void SpawnAIAtCurrentLocationDelayed(const FString& AIDescString, float Delay);
	void SpawnAI(const FString& AIDescString);
	...
	void SinkShipByActorId(const FString& ShipActorIdString);
	void SinkShip();
	...
	void SimulatePetReactRequest(const FString& Id);
	void SimulatePetReactCancellation(const FString& Id);
	...
	void ShowAllianceStatus(const FString& CrewId);
	void ShipwrecksSpawnOne(float SpawnLocationX, float SpawnLocationY, float SpawnLocationZ);
	void ShipwrecksSpawnCustom(float SpawnLocationX, float SpawnLocationY, float SpawnLocationZ, const FString& WreckAsset);
	...
	void SetDebugItemSource(const FString& Source);
	...
	void SetAxisBinding(const FString& InBindingName, const FString& InKeyName);
	void SetAllAIOverrideCannonShotHitChance(float HitChance);
	void SetAITeamAttitude(const FString& TeamAString, const FString& TeamBString, const FString& AttitudeString);
	void SetAIExclusiveAbility(const FString& AIAbilityString);
	void SetAIAbilityTimeMultiplier(const FString& AIAbilityString, float IntervalCooldownMultiplier, float ActivationCooldownMultiplier);
	void SetAbilityAlwaysOn(const FString& AIAbilityString);
	void SendUpdateVoyageProgressEvent();
	void SendStatEvent(uint32_t StatId, uint64_t StatValue);
	void SendRewardRequestEvent(const FString& CompanyNameAndRewardIdSeparatedByColon);
	...
	void RetrieveItemsFromBootyStorage(const FString& IslandName);
	void ResurfaceShipByActorId(const FString& ShipActorIdString);
	...
	void ReplenishShip();
	void ReplaceShipWithSmallShip(const FString& ShipActorIdConsoleString);
	void RepairShipAndClearInternalWater();
	void RepairAndClearInternalWaterOnAllShips();
	void RenameTreasure(const FString& InVendorName);
	...
	void RemoveAllFog();
	void RemoveAISpawnContext(const FString& ContextName);
	...
	void PlayWorldSequence(const FString& InReference);
	void PlayLocalSequences();
	void PlayerAnimationOverride(const FName& Name);
	void OverrideShipPartFromCatalogue(const FString& InShipActorIdConsoleString, int InCataloguePartIndex, int InCataloguePartCustomisationIndex);
	void OpenSkeletonFortDoor();
	void MoveStormToPlayer();
	void MoveStormToLocation(float LocationX, float LocationY);
	void MoveStormToIsland(const FString& IslandNameString);
	...
	void LockTradeRouteSelectionToSpecificRouteForMyCrew(const FString& RouteReferencePath);
	void LockTradeRouteSelectionToSpecificRouteForCrew(const FGuid& CrewId, const FString& RouteReferencePath);
	void LocallyUnBreakLeg();
	void LocallyDisableTutorial();
	void LoadMyShip();
	void LightLocalBraziers();
	void LightBraziersInRadius(float Radius);
	void LeaveAlliance(const FString& CrewId);
	...
	void KrakenAnimatedTentacleChangePlayerHoldState(const FString& HoldState);
	void KindleClosestShip();
	void KindleAllShipFires();
	void KillPlayer();
	void KillCrew(const FString& CrewId);
	...
	void JoinAlliance(const FString& OfferingCrew, const FString& AcceptingCrew);
	void IPGOverride(const FName& BodyShape, float Distance);
	void IPGLoadWithoutClothing(const FString& ActorIdString, const FString& path);
	void IPGLoadWithClothing(const FString& ActorIdString, const FString& path);
	void IPGLoad(const FString& path);
	...
	void HealthContinuousStopWithReason(const FString& Reason);
	void HealthContinuousStartWithTestReason(float Value);
	void HealthContinuousStartWithReason(float Value, const FString& Reason);
	void HealthAdjust(float Amount);
	void God();
	void ForceStopAllPetsDanger();
	void ForceStartAllPetsDangerWithChangingThreatLocation(const FString& ResponseType, float UpdateThreatLocationTime);
	void ForceStartAllPetsDanger(const FString& ResponseType);
	void ForceSkipTallTaleSteps_WaitAndCutscenes();
	void ForceRequestCampaignsFromServices();
	void ForcePetHangout(const FName& HangoutName, int PositionIndex);
	void ForceOpenShop();
	...
	void ForceEmoteWithDescription(const FName& EmoteIdentifier, const FString& DisplayName, const FString& AudioDescription);
	void ForceEmote(const FName& EmoteIdentifier);
	void ForceCloseShop();
	void ForceAllPetsDangerWithNoiseEventWithChangingThreatLocation(const FString& ResponseType, float UpdateThreatLocationTime);
	void ForceAllPetsDangerWithNoiseEvent(const FString& ResponseType);
	...
	void FireSeasonGoalProgressionMessage(int InNumSeason, int InPreviousProgress, int InCurrentProgress, int InCompletionTreshold, int InGoalType, const FString& InGoalCategory);
	void FireSeasonGoalCompletionMessage(int InNumSeason, int InGoalType, const FString& InGoalCategory);
	...
	void FindText(const FString& Namespace, const FString& Key);
	void FakeMigrateBountyQuests();
	void EquipPirateTitle(const FString& PirateTitleType);
	...
	void DrawVideprinter(const FString& Id, bool Active);
	...
	void DiveShipByActorId(const FString& ShipActorIdString);
	void DisplaySpireLocationsDebug(int Enabled);
	void DisplaySingleEmblemProgress(const FString& StatName);
	...
	void DioramaStartNearest(const FString& Spawner, const FString& DioramaDesc);
	void DioramaStart(const FString& ActorName, const FString& Spawner, const FString& DioramaDesc);
	void DioramaKillAllDebug();
	void DestroyShip(const FString& ShipActorIdConsoleString);
	...
	void DespawnNumberOfAI(const FString& AITypeString, int NumToDespawn);
	void DespawnFirstAI();
	void DespawnAllAggressiveGhostShips();
	void DespawnAI(const FString& AITypeString);
	void DeleteVoyageHistory();
	void DeleteAllMermaids();
	void DecrementDebugHealthStage();
	void DebugIslandDelta();
	void DeactivateSkellyFort(const FString& FortName);
	void DeactivateEmissaryFlagCompany();
	void DeactivateDeathEffect();
	void DamageShipFromRemoteActor(const FString& ActorIdString, float Strength, float CameraLocationX, float CameraLocationY, float CameraLocationZ, float CameraForwardDirectionX, float CameraForwardDirectionY, float CameraForwardDirectionZ);
	void DamageShip(float Strength);
	void DamagePlayerFromRemoteActor(const FString& ActorIdString, float Strength, float CameraLocationX, float CameraLocationY, float CameraLocationZ, float CameraForwardDirectionX, float CameraForwardDirectionY, float CameraForwardDirectionZ);
	void DamagePlayer(float Strength);
	void CureAllAilings();
	void CreateDebugReapersChestMarkerAtPlayerLocation();
	void CreateDebugAISpawnerAt(const FString& SpawnerAssetName, const FString& LocationActorName);
	void CreateDebugAISpawner(const FString& SpawnerAssetName);
	void CoordinatedKrakenToggleAI();
	void CoordinatedKrakenRequestAction(const FString& ActionName);
	...
	void ClearAxisBinding(const FString& InBindingName);
	...
	void BuryItem(const FString& NameOfItemToBury);
	void BreakLeg();
	void BlockMigrationForPlayer(bool Enabled);
	void ApplyStatusToPlayer(const FString& StatusIdentifier, float Duration);
	void ApplyDamageToAllDamageZones(float Damage);
	void ApplyCursedCannonballStatusToShip(const FString& CannonballTypeString);
	void ApplyCursedCannonballStatusToPlayer(const FString& CannonballTypeString);
	void AllowTeleportWithItems(bool CanTeleport);
	void AllocatePortalToTunnelOfTheDamned(const FString& TunnelDescAssetPath);
	...
	void AddShipToCrew(const FString& ActorIdString, const FString& CrewId);
	void AddRandomPetForAllPlayers();
	void AddPlayerToCrew(const FString& ActorIdString, const FString& CrewId);
	void AddPetForPlayer(int PetTypeIndex, int PetPartIndex);
	void AddPetForAllPlayers(int PetTypeIndex, int PetPartIndex);
	void AddMysteriousNote(const FString& NoteType, const FString& NoteTitle, const FString& NoteBody, const FString& CompletionStrings);
	void AddDrunkenness(int DrunkennessType, float DrunkennessChange);
	void AddAISpawnContext(const FString& ContextName);
	void ActivateSkellyFort(const FString& FortEventName, const FString& FortName);
	void ActivateEmissaryFlagCompany(const FString& CompanyId);
